...
df = df.dropna()
#len(df)
#df.to_csv("C:\\Users\\Vikram.V.Kumar\\Desktop\\df.csv")
...
ch = pd.read_csv("C:\\Users\\Vikram.V.Kumar\\Desktop\\Advanced Analytics Practice\\Hackathon\\Data\\Charging_Stations.csv")
...
for i in range(0,106):
    x=ch['latitude']
    x=x.to_frame()
    x['latitude'] = ch['latitude'][i]
    y=ch['longitude']
    y=y.to_frame()
    y['longitude'] = ch['longitude'][i]
    z=ch['Station_ID'][i]
    ch[z]=0
    for row in ch.itertuples(index=False):
        ch[z]=haversine_distance(x['latitude'],y['longitude'], ch['latitude'],ch['longitude'])    
    if(('Shell'not in z) & (ch[z].nsmallest(2).iloc[-1]<1.7)):
        ch['station_competitor'][i]=1
        
ch.to_csv("C:\\Users\\Vikram.V.Kumar\\Desktop\\Charging_Stations_distances.csv")
